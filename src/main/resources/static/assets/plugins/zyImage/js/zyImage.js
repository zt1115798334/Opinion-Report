		...
		return this.each(function(){  
			...
			this.loadZoomJsCss = function(){
				...
				if(navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.match(/8./i)=="8."){
					...
					$.getScript("/assets/plugins/zyImage/zoomImage/js/e-smart-zoom-jquery.js", function(){
						console.info("zoomImageJS加载完成")
					});
					$("<link>").attr({ rel: "stylesheet",
				        type: "text/css",
				        href: "/assets/plugins/zyImage/zoomImage/css/styles.css"
				    }).appendTo("head");
				}
			};
			...
			this.createMainImageHtml = function(){
				...
				html += '		<span id="studyJudge"><a href="###" style="display: inline-block;color: #fff;" data-toggle="modal" data-target="#judge">研判为 </a></span>'
			...
			this.bindThumImageEvent = function(){
				...
				console.log($(self).find(".mc-thumb-frame")[0]);
				// 绑定每个缩略图的单击事件
				$(self).find(".mc-thumb-frame").on("click", function(e){
					// 首先判断当前是不是已经选中
					var $thum = $(this);
